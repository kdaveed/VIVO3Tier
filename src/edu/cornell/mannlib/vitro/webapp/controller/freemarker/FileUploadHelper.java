...
import static edu.cornell.mannlib.vitro.webapp.controller.freemarker.FileUploadController.PARAMETER_UPLOADED_FILE;
import static edu.cornell.mannlib.vitro.webapp.controller.freemarker.FileUploadController.THUMBNAIL_HEIGHT;
import static edu.cornell.mannlib.vitro.webapp.controller.freemarker.FileUploadController.THUMBNAIL_WIDTH;
...
import edu.cornell.mannlib.vitro.webapp.controller.freemarker.FileUploadController.UserMistakeException;
...
public class FileUploadHelper {
	...
	private static final String ERROR_CODE_UNRECOGNIZED_FILE_TYPE = "imageUpload.errorUnrecognizedFileType2";
	...
	private static final Map<String, String> RECOGNIZED_DOCUMENT_TYPES = createDocumentTypesMap();
	...
	private static Map<String, String> createDocumentTypesMap() {
		Map<String, String> map = new HashMap<String, String>();
		map.put(".pdf", "text/pdf");
		map.put(".txt", "text/plain");
		map.put(".doc", "text/doc");
		map.put(".xls", "text/xls");
	...
	FileUploadHelper(FileStorage fileStorage,
	...
	FileItem validateImageFromRequest(VitroRequest vreq)
		...
		log.info("Map<String, List<FileItem>> map = vreq.getFiles();  :  "  + map);
	...
	List<FileItem> validateImagesFromRequest(VitroRequest vreq)
			throws UserMistakeException {
		Map<String, List<FileItem>> map = vreq.getFiles();
		if (map == null) {
			throw new IllegalStateException(ERROR_CODE_BAD_MULTIPART_REQUEST);
	...
	List<FileItem> validateDocumentsFromRequest(VitroRequest vreq)
			throws UserMistakeException {
		Map<String, List<FileItem>> map = vreq.getFiles();
		if (map == null) {
			throw new IllegalStateException(ERROR_CODE_BAD_MULTIPART_REQUEST);
		...
		for(int i=0; i<list.size(); i++){
			...
			if (!RECOGNIZED_DOCUMENT_TYPES.containsValue(mimeType)) {
				log.debug("Unrecognized MIME type: '" + mimeType + "'");
				throw new UserMistakeException(ERROR_CODE_UNRECOGNIZED_FILE_TYPE,
						filename, "DOC, PDF, TXT or XLS");
			}
		}
		return list;
	}
	 ...
	 */
	FileInfo storeNewImageIndividual(FileItem fileItem, VitroRequest vreq) {
	...
	FileInfo storeNewFileIndividual(FileItem fileItem, VitroRequest vreq) {
		InputStream inputStream = null;
		try {
			inputStream = fileItem.getInputStream();
			log.info("InputStream : " + inputStream);
	...
	void storeImageToIndividual(Individual entity, FileInfo newImage) {
		
		uploadedFileHelper.saveHasImageEntity(entity.getURI(), newImage);
	}
	
	void storeDocumentToIndividual(Individual entity, FileInfo newDocument) {
		
		uploadedFileHelper.saveHasDocumentEntity(entity.getURI(), newDocument);
	}
