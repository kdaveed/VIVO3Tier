...
public class FileUploadController extends FreemarkerHttpServlet {
	...
	private static final String ATTRIBUTE_REFERRING_PAGE = "FileUploadController.referringPage";
	...
	/** Edit the images	**/
	public static final String ACTION_EDIT_IMAGES = "editImages";
	...
	/**	This form paramater defined in the imageUpload-Individual.ftl	**/
	public static final String ACTION_IMAGE_UPLOAD = "imageUpload";
	public static final String ACTION_DOCUMENT_UPLOAD = "documentUpload";
	...
	public static final String TEMPLATE_SHOW_UPLOADED_IMAGES = "imageUpload-showUploadedImages.ftl";
	public static final String TEMPLATE_SHOW_UPLOADED_DOCUMENTS = "imageUpload-showUploadedDocuments.ftl";
	...
	protected ResponseValues processRequest(VitroRequest vreq) {
		...
		try {
			...
			} else if (ACTION_IMAGE_UPLOAD.equals(action)) {
				
				/*		
				 * 		We are here from the imageUpload-fileToIndividual
				 */
				return doUploadImages(vreq, entity);
			
			} else if (ACTION_DOCUMENT_UPLOAD.equals(action)) {
				
				/*		
				 * 		We are here from the imageUpload-fileToIndividual
				 */
				return doUploadDocuments(vreq, entity);
	...
	private ResponseValues doUploadImage(VitroRequest vreq, Individual entity) {
		FileUploadHelper helper = new FileUploadHelper(fileStorage,
		...
		try {
			...
			FileInfo fileInfo = helper.storeNewImageIndividual(fileItem, vreq);
	...
	private List<Map<String,Object>> getImagesMap(Individual ind){
		
		List<Map<String,Object>> mapList= new ArrayList<Map<String, Object>>();
		List<Individual> ind1 = ind.getRelatedIndividuals("http://files.org#hasImage");
		
		for(int i = 0; i<ind1.size(); i++){
			
			log.info("individual " + i);
			Map<String, Object> modelMap = new HashMap<String, Object>();
			Individual ind2 = ind1.get(i).getRelatedIndividual(VitroVocabulary.FS_DOWNLOAD_LOCATION);
			modelMap.put("src", "/vivo" + ind2.getDataValue(VitroVocabulary.FS_ALIAS_URL));
			modelMap.put("uri", ind1.get(i).getURI());
	...
	private ResponseValues doUploadImages(VitroRequest vreq, Individual entity) {
		FileUploadHelper helper = new FileUploadHelper(fileStorage,
				vreq.getUnfilteredWebappDaoFactory(), getServletContext());

		
		User usr =  new User(vreq);
        log.info("UserProfileUrl :  " + usr.getProfileUrl());
		
		HttpSession session = vreq.getSession();
		log.info("Session object : " + session.getAttribute("userId"));
		
		try {
			// Did they provide a file to upload? If not, show an error.

			List<FileItem> fileItem = helper.validateImagesFromRequest(vreq);

			// Put it in the file system, and store a reference in the session.
			List<Map<String, Object>> imageByteStreams = new ArrayList<Map<String,Object>>();
			
			for(FileItem i : fileItem){
				
				FileInfo imageInfo = helper.storeNewFileIndividual(i, vreq);
				...
				a.put("src", "/vivo" + imageInfo.getBytestreamAliasUrl());
				a.put("uri", imageInfo.getUri());
				imageByteStreams.add(a);
			}

			// Go to the cropping page.
			return showUploadedImages(vreq, entity,imageByteStreams);
		} catch (UserMistakeException e) {
			return showErrorMessage(vreq, entity, e.formatMessage(vreq));
		}
	}
	...
	private ResponseValues doUploadDocuments(VitroRequest vreq, Individual entity) {
		FileUploadHelper helper = new FileUploadHelper(fileStorage,
				vreq.getUnfilteredWebappDaoFactory(), getServletContext());

		
		HttpSession session = vreq.getSession();
		session.setAttribute("userId", "kutyaLó");
		log.info("Session object : " + session.getAttribute("userId"));
		/*
		...
		List<Individual> inds = SelfEditingConfiguration.getBean(vreq)
				.getAssociatedIndividuals(indDao, c);
		if (inds.isEmpty()) {
		...
		*/
		...
		try {
			...
			List<FileItem> fileItem = helper.validateDocumentsFromRequest(vreq);

			// Put it in the file system, and store a reference in the session.
			List<Map<String, Object>> documentByteStreams = new ArrayList<Map<String,Object>>();
			
			for(FileItem i : fileItem){
				
				FileInfo documentInfo = helper.storeNewFileIndividual(i, vreq);

				helper.storeDocumentToIndividual(entity, documentInfo);
				Map<String,Object> a = new HashMap<String,Object>();
				a.put("src", "/vivo" + documentInfo.getBytestreamAliasUrl());
				a.put("fileName", documentInfo.getFilename());
				a.put("uri", documentInfo.getUri());
				documentByteStreams.add(a);
			}

			// Go to the cropping page.
			return showUploadedDocuments(vreq, entity,documentByteStreams);
		} catch (UserMistakeException e) {
			return showErrorMessage(vreq, entity, e.formatMessage(vreq));
		}
	}
	
	private ResponseValues showEditUploadedImages(VitroRequest vreq, Individual entity) {
		FileUploadHelper helper = new FileUploadHelper(fileStorage,
				vreq.getUnfilteredWebappDaoFactory(), getServletContext());
		...
		try {
			...
			for(FileItem i : fileItem){
				...
				FileInfo imageInfo = helper.storeNewImageIndividual(i, vreq);
	...
	private TemplateResponseValues showUploadedImages(VitroRequest vreq,
			Individual entity, List<Map<String,Object>> imageUrl) {
		TemplateResponseValues rv = new TemplateResponseValues(TEMPLATE_SHOW_UPLOADED_IMAGES);
		
		rv.put("fromUpload","etwas");
		/*	Here the map for the images	*/
		
		//rv.put(BODY_MAIN_IMAGE_URL, UrlBuilder.getUrl(imageUrl));
		rv.put("individualUri", entity.getURI());
		rv.put("predicateUri", "http://files.org#hasImage");
		rv.put("images", imageUrl);
		rv.put(BODY_FORM_ACTION, UrlBuilder.getIndividualProfileUrl(entity, vreq));
		rv.put(BODY_CANCEL_URL, exitPageUrl(vreq, entity.getURI()));
		rv.put(BODY_TITLE, figureCropPageTitle(vreq, entity));
		return rv;
	}
	
	private TemplateResponseValues showUploadedDocuments(VitroRequest vreq,
			Individual entity, List<Map<String,Object>> documentUrl) {
		TemplateResponseValues rv = new TemplateResponseValues(TEMPLATE_SHOW_UPLOADED_DOCUMENTS);
		...
		//rv.put(BODY_MAIN_IMAGE_URL, UrlBuilder.getUrl(imageUrl));
		rv.put("individualUri", entity.getURI());
		rv.put("predicateUri", "http://files.org#hasDocument");
		rv.put("documents", documentUrl);
		rv.put(BODY_FORM_ACTION, UrlBuilder.getIndividualProfileUrl(entity, vreq));
		rv.put(BODY_CANCEL_URL, exitPageUrl(vreq, entity.getURI()));
		rv.put(BODY_TITLE, figureCropPageTitle(vreq, entity));
		return rv;
	...
	private Individual validateEntityUri(VitroRequest vreq)
		...
		log.info("Individual entity  : " + entity);
	...
	private TemplateResponseValues showAddImagePage(VitroRequest vreq,
		...
		log.info("TemplateResponseValues map : " + rv.getMap());
		return rv;
	}
